...
import cyd.ace.acetypes.*;
import cyd.classifiers.perceptron.AbstractInstance;
import cyd.commons.Alphabets;
import cyd.commons.Sentence;
import cyd.commons.Sentence.Sent_Attribute;
import cyd.event.perceptron.featureGenerator.NodeFeatureGenerator;
...
public class SentenceInstance extends AbstractInstance
{
	...
	public SentenceInstance(Alphabets alphabets, 
	...
	public List<AceEvent> getEvents(SentenceAssignment assn, String id, String fileText)
	{
		...
		for(int i=0; i<assn.nodeAssignment.size(); i++)
		{
			...
			String label = (String) this.alphabets.nodeTargetAlphabet.lookupObject(trigger_label);
			if(label != null && !label.equals(SentenceAssignment.Default_Trigger_Label))
			{
				...
				if(edgeMap != null)
				{
					for(Integer arg_index : edgeMap.keySet())
					{
						...
						String role = (String) this.alphabets.edgeTargetAlphabet.lookupObject(role_index);
	...
	public boolean violateGoldStandard(SentenceAssignment assn)
	{
		...
		return !assn.equals((SentenceAssignment) this.target, this.target.state);
	}
	...
	public boolean violateGoldStandard(List<SentenceAssignment> beam, int argNum)
	{
		for(SentenceAssignment assn : beam)
		{
			if(assn.equals((SentenceAssignment) this.target, assn.state, argNum))
			{
				return false;
			}
		}
