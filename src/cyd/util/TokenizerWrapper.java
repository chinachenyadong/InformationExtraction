...
public class TokenizerWrapper 
{
	 ...
	 * @return
	 */
	public cyd.util.Span[] tokenizeSpan(String text)
	{
		Span[] spans = tokenizer.tokenizePos(text);
		cyd.util.Span[] ret = new cyd.util.Span[spans.length];
		int i=0;
		for(Span span : spans)
		{
			cyd.util.Span cunySpan = new cyd.util.Span(span.getStart(), span.getEnd() - 1);
			ret[i++] = cunySpan;
		}
		return ret;
	}
