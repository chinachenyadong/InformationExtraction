...
public class SentDetectorWrapper 
{
	...
	public cyd.util.Span[] detectPosMonocase(String text)
	{ 
	 ...
	 * @return
	 */
	public cyd.util.Span[] detectPos(String text)
	{
		ArrayList<cyd.util.Span> ret = new ArrayList<cyd.util.Span>();
		...
		return ret.toArray(new cyd.util.Span[ret.size()]);
	}

	private void segBuffer(ArrayList<cyd.util.Span> ret, String buffer, int offset)
	{
		Span[] spanBuffer = sentDetector.sentPosDetect(buffer);
		for(int i=0; i<spanBuffer.length; i++)
		{
			Span span = spanBuffer[i];
			cyd.util.Span adjustedSpan = new cyd.util.Span(span.getStart() + offset, span.getEnd() - 1 + offset);
	...
	public static void main(String[] args) throws InvalidFormatException, IOException
	{
		...
		cyd.util.Span[] spans = SentDetectorWrapper.getSentDetector().detectPosMonocase(text);
		
		for(cyd.util.Span span : spans)
		{
